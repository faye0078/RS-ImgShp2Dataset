...
def ReadFileBytxt(txtfilename):
    ...
    for file_name in file_list:
        file_path = "E:/wangyu_file/rs_Nas/data/512/label/" + file_name.split(".")[0] + "/"
        imgs = glob.glob('{}*.png'.format(file_path))
        for img in imgs:
            img = img.replace('E:/wangyu_file/rs_Nas/data/512/', '')
            file.append(img)
    df = pd.DataFrame(file, columns=['one'])
    df.to_csv("E:/wangyu_file/2.lst", columns=['one'], index=False, header=False)
...
if __name__ == '__main__':
    ...
    mergeFile()
    # changeFile()
    # path = "E:/wangyu_file/rs_Nas/src/data/lists/gid_testlist.txt"
    # ReadFileBytxt(path)