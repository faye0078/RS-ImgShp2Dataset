...
def main():
    ...
    def create_segmenter(encoder):
        ...
        with torch.no_grad():
            ...
            # decoder_config = [[8, [0, 0, 9, 1], [2, 1, 10, 8], [4, 1, 3, 9]], [[0, 3], [3, 2], [4, 1]]]
            # decoder_config = [[6, [0, 0, 9, 6], [0, 0, 10, 9], [0, 5, 4, 6]], [[2, 3], [0, 0], [0, 2]]]
            decoder = Decoder(
    ...
    for epoch in range(epoch_start, args.num_epochs):
        ...
        for task_idx in range(args.num_tasks):
            ...
            for epoch_segm in range(args.num_segm_epochs[task_idx]):
                ...
                if (epoch_segm + 1) % (args.val_every[task_idx]) == 0:
                    ...
                    # if task_miou > avr[task_idx].avg:
                    if task_miou > best_task0_miou[task_idx]:
                        best_task0_miou[task_idx] = task_miou
                        # avr[task_idx].update(task_miou)
                        # if task_miou > best_reward and task_idx == 1:
                            ...
                            # if task3_miou > task_miou:
                            #     true_best_miou = task3_miou
                            #     print("true_best_miou = " + str(true_best_miou))
                            #     PATH = "E:/wangyu_file/model/best_hrnet_decoder.pth"
                            #     torch.save(segmenter.state_dict(),PATH)
                            # best_reward = task_miou

                        # print("auix_best_miou = " + str(best_reward))
                    else:
        ...
        # do_search = False
        if do_search:
            print("number change xytrain = " + str(num_change_xy))
