...
def image_clip(img_path, size):
    ...
    img_dir = "/media/dell/DATA/wy/data/Guangdong/dataset/512/image/" + img_name
    ...
    for row in range(num_row):
        ...
        clipImg = Image.fromarray(clipArray)
        img_filepath = img_dir + '/' + img_name + "_" + str(
            row + 1) + "_" + str(num_col + 1) + "_img.png"
        clipImg.save(img_filepath)
        ...
        clipImg = Image.fromarray(clipArray)
        img_filepath = img_dir + '/' + img_name + "_" + str(
            row + 1) + "_" + str(num_col + 2) + "_img.png"
        clipImg.save(img_filepath)
    ...
    for col in range(num_col):
        ...
        clipImg = Image.fromarray(clipArray)
        img_filepath = img_dir + '/' + img_name + "_" + str(
            num_row + 1) + "_" + str(col + 1) + "_img.png"
        clipImg.save(img_filepath)
    ...
    clipImg = Image.fromarray(clipArray)
    img_filepath = img_dir + '/' + img_name + "_" + str(
        num_row + 1) + "_" + str(num_col + 2) + "_img.png"
    clipImg.save(img_filepath)
    ...
    clipImg = Image.fromarray(clipArray)
    img_filepath = img_dir + '/' + img_name + "_" + str(
        num_row + 2) + "_" + str(num_col + 1) + "_img.png"
    clipImg.save(img_filepath)
    ...
    clipImg = Image.fromarray(clipArray)
    img_filepath = img_dir + '/' + img_name + "_" + str(
        num_row + 2) + "_" + str(num_col + 2) + "_img.png"
    clipImg.save(img_filepath)
def label_clip(img_path, size):

    # 转换为数组进行分割操作，计算能完整分割的行数(row)、列数(col)
    img_name = img_path.split('.')[-2].split('/')[-1].replace('_label', '')
    img_dir = "/media/dell/DATA/wy/data/Guangdong/dataset/512/label/" + img_name
    folder = os.path.exists(img_dir)
    if not folder:
        os.makedirs(img_dir)
...
if __name__=='__main__':
    folder = os.path.exists("/media/dell/DATA/wy/data/Guangdong/dataset/512/label")
    if not folder:
        os.makedirs("/media/dell/DATA/wy/data/Guangdong/dataset/512/label")

    img_dir = '/media/dell/DATA/wy/data/Guangdong/dataset/origin_label/'
    ...
    #
    # img_dir = '/media/dell/DATA/wy/data/Guangdong/dataset/origin_image/'
    # imgs = glob.glob('{}*.bmp'.format(img_dir))
    # for img in imgs:
